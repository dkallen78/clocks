...
function makeNumbers(parent, number) {
  ...
  for (let i = 0; i < number; i++) {
    ...
    numero.textContent = digit.toString(10).padStart(2, 0);
  ...
  setTimeout(function() {
    ...
    let msTotal = (time.getHours() * 3600000) + msMinutes;
    illuminateHour(Math.floor(msTotal / 8640000));
  }, 100);
...
function illuminateHour(hour) {
  ...
  let newHour = ((hour + 10) - 1) % 10;
  let oldHour = ((hour + 10) - 2) % 10;
  let numbers = document.getElementsByClassName("numbers");
...
let refreshInterval = setInterval(function() {
  ...
  let sAngle = ((msTotal / 8.64) % 1000000) * 0.036;
  ...
  let mAngle = ((msTotal / 864) % 10000) * 0.036;
  ...
  let hAngle = msTotal * 0.000004166;
  ...
  let timeString = (msTotal / 8640000).toPrecision(5);
  let hour = Math.floor(msTotal / 8640000);
